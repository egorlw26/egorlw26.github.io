class FirstBestQElement{
	constructor(element, h, parent){
    this.element = element;
		this.h = h;
    this.priority = this.h;
    this.parent = parent;
  }
}

class FirstBestPathfinder extends Pathfinder{
	...
	preparate(){
		...
		var startEl = new FirstBestQElement(this.start, 0, 0);
		startEl.priority = 0;
	...
	getSearchStep(){
		if(!this.open.isEmpty() && !this.finish){
			...
			for(let i = 0; i < availableNeighbours.length; ++i) {
				...
				var element = new FirstBestQElement(neigh, h, currCell);
